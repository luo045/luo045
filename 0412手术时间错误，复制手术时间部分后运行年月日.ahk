...
if RegExMatch(time1, "(\d{4})年(\d{2})月(\d{2})日(\d{2}):(\d{2})", match)
yyyy := match1
mm := match2
dd := match3
hh := match4
...
if RegExMatch(time2, "(\d{4})年(\d{2})月(\d{2})日(\d{2}):(\d{2})", match)
    yyyy := match1
mm := match2
dd := match3
hh := match4
...
FillDateTimeField(Text:="|<>*163$55.0C0P10U1UTs0AEbzzzkM060Y80M0A3zzw43znzw1U4zl68301s600X41U1O3rwTyTzlAX221s0M36A111O0A333CUXAk601UMTm66S00k08830U", {x1:10, y1:200, x2:1060, y2:900}, yyyy, mm, dd, hh, min) ;
; 通用日期时间填写函数
